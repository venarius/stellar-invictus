class StructuresController < ApplicationController
        ...
        if items.present? && (structure.location == current_user.location)
          ...
          if count > 0
            if params[:loader] && (count == item_count)
              ...
              render(json: { amount: item_count - free_weight }, status: 200) && (return)
            end
          else
  ...
  def abandoned_ship
    if params[:id] || (params[:id] && params[:text])
      structure = Structure.find(params[:id]) rescue nil
      if structure && (structure.location == current_user.location) && current_user.can_be_attacked
        if params[:text] && structure.items.present?
          ...
          render(partial: 'structures/abandoned_ship', locals: { structure: structure }) && (return)
        end
      end
    end
    render json: {}, status: 400
  ...
  def monument_info
    if params[:id]
      structure = Structure.find(params[:id]) rescue nil
      if structure && (structure.location == current_user.location) && current_user.can_be_attacked
        render(partial: 'structures/monument', locals: { structure: structure }) && (return)
      end
    end
    render json: {}, status: 400
  end
