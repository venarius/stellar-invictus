class AdminController < ApplicationController
  ...
  def ban
    if params[:id] && params[:duration] && params[:reason]
      ...
      render(json: { message: I18n.t('admin.successfully_banned_user'), banned_until: (user.banned_until.strftime("%F %H:%M") rescue 0) }, status: 200) && (return)
    end
    render json: {}, status: 400
  end
  ...
  def unban
    @user.unban
    render(json: { message: I18n.t('admin.successfully_unbanned_user') }, status: 200) && (return)
  end
  ...
  def mute
    ...
    render(json: { message: I18n.t('admin.successfully_muted_user') }, status: 200) && (return)
  end

  def unmute
    @user.update_columns(muted: false)
    render(json: { message: I18n.t('admin.successfully_unmuted_user') }, status: 200) && (return)
  end

  def delete_chat
    @user.chat_messages.destroy_all
    render(json: { message: I18n.t('admin.successfully_deleted_chat') }, status: 200) && (return)
  end
