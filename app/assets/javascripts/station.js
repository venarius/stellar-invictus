$(document).on("turbolinks:load", function() {
  ...
  $(".station-card").on("click", ".store-all-btn", function(e) {
    ...
    var jqxhr = $.post(
      "/stations/store",
      { loader: $(this).data("loader"), amount: $(this).data("amount") },
      function() {
        ...
        if (button.closest("tbody").find("tr").length == 1) {
          button
            .closest("table")
            .replaceWith("<h2 class='text-center'>...</h2>");
        } else {
          button
            .parent()
            .parent()
  ...
  $(".station-card").on("click", "#store-modal .store-confirm-btn", function(
    e
  ) {
    var button = $(this);
  ...
  $(".station-card").on("click", "#load-modal .load-confirm-btn", function(e) {
    ...
    jqxhr.fail(function(data) {
      $("#load-modal")
        ...
        .addClass("outline-danger");
      $("#load-modal")
        .find("span.text-center.color-red")
        .remove();
      $("#load-modal")
        .find(".input-group")
        .after(
          "<span class='text-center color-red'>" +
  ...
  $(".station-card").on("click", ".craft-btn", function() {
    ...
    $.post(
      "factory/craft",
      { loader: loader, type: type, amount: amount },
      function() {
        button.closest(".modal").modal("hide");
        setTimeout(function() {
  ...
  $("#factory").on("click", ".factory-modal-btn", function() {
    ...
    if (loader && type) {
      ...
      $.get("factory/modal", { loader: loader, type: type }, function(data) {
        ...
        if (data && data.responseJSON.error_message) {
          $.notify(data.responseJSON.error_message, { style: "alert" });
        }
        button.html(html);
      });
      ...
      .each(function() {
        $(this).html(
          parseInt($(this).data("amount")) *
            parseInt($("#dismantleModalInput").val())
        );
      });
  });
  ...
  $("#factory").on("click", "#dismantleModalBtn", function() {
    ...
    $.post("/factory/dismantle", { amount: amount, loader: loader }, function(
      data
    ) {
      button.html(html);
