$( document ).on('turbolinks:load', function() {
  ...
  $('#add-friend-modal').on('click', '#add-friend-modal-search-btn', function(e) {
    if ($('#add-friend-modal-input').val()) {
        $('#add-friend-modal-input').css("border", "1px solid grey");
        $('#add-friend-modal-result').empty().append("<div class='text-center spinner-modal'><i class='fa fa-spinner fa-spin fa-2x'></i></div>")
      $.post('friends/search', {name: $('#add-friend-modal-input').val()}, function(data) {
        $('#add-friend-modal-body').find('.spinner-modal').remove();
        $('#add-friend-modal-result').empty().append(data);
      })
    } else {
  ...
  $('#add-friend-modal').on('click', '.search-remove-as-friend-btn', function(e) {
    e.preventDefault();
    var id = $(this).data('id');
    $.post('friends/remove_friend', {id: id}, function(data){
      $('#add-friend-modal').modal('hide');
