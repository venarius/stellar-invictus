$(document).on("turbolinks:load", function() {
  ...
  $("#app-container").on("click", "#diceMax", function() {
    ...
    if (parseInt($("#diceBet").val()) > 100000) {
      $("#diceBet").val("100000");
    }
    $("#diceProfit").val(
      parseInt(
  ...
  $("#app-container").on("click", "#diceRoll", function() {
    ...
    if (bet && roll_under) {
      $.post(
        "/stations/dice_roll",
        { bet: bet, roll_under: roll_under },
        function(data) {
          if (data.win) {
            $("#diceHistory").append(
              "<tr class='color-highgreen'><td>" +
                data.time +
                "</td><td>" +
                data.bet +
                "</td><td>" +
          ...
          $(".player-info-card")
            .find(".fa-dollar-sign")
            .parent()
            .html("<i class='fa fa-dollar-sign'></i>&nbsp;&nbsp;" + data.units);
