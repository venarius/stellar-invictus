$(document).on("turbolinks:load", function() {
  // Load Tab
  if (
    window.location.pathname == "/corporation" &&
    $(".corporation-card").find(".nav").length
  ) {
    load_corporation_tab(
      $(".corporation-card a.nav-link.active").data("target")
    );
  }
  ...
  $(".corporation-card").on("click", "#edit-corporation-motd-btn", function() {
    ...
    if ($("#corporation-motd").find("textarea").length) {
      ...
      $("#corporation-motd").html(
        "<textarea class='form-control' id='corporation-motd-editarea'>" +
          old +
          "</textarea>"
      );
  ...
  $(".corporation-card").on("click", "#edit-corporation-btn", function(e) {
    ...
    if (
      ...
      // Save Mode
      var tax = $("#corporation-form")
        .find("input")
        .val();
      var about = $("#corporation-form")
        .find("textarea")
        .val();
      $.post(
        "corporation/update_corporation",
        { tax: tax, about: about },
        function(data) {
          $("#corporation-form")
            .find(".edit-view")
  ...
  $(".corporation-card").on("click", ".corporation-kick-user-btn", function() {
    ...
    if (result) {
      $.post("corporation/kick_user", { id: button.data("id") }, function(
        data
      ) {
        if (data.reload == true) {
  ...
  $(".corporation-card").on("click", ".corporation-mote-user-btn", function(e) {
    $.get("corporation/change_rank_modal", { id: $(this).data("id") }, function(
      data
    ) {
      $(data)
  ...
  $("#app-container").on(
    "click",
    "#corporation-change-rank-modal .corporation-save-mote-user",
    function() {
      var rank = $("#corporation-change-rank-modal")
        .find(":selected")
        .val();
      var button = $(this);

      $.post(
        "corporation/change_rank",
        { id: button.data("id"), rank: rank },
        function(data) {
          button.closest(".modal").modal("hide");
          setTimeout(function() {
            load_station_tab("#roster");
          }, 250);
        }
      ).fail(function(data) {
  ...
  $(".corporation-card").on(
    "click",
    ".corporation-deposit-credits-btn",
    function(e) {
      var button = $(this);
      var amount = button
        .closest(".modal")
      ...
      $.post("corporation/deposit_credits", { amount: amount }, function(data) {
        ...
        if (!button.closest(".modal").find(".error").length) {
          button
            ...
            .find(".modal-body")
            .after(
              "<span class='color-red text-center mb-3 error'>" +
  ...
  $(".corporation-card").on(
    "click",
    ".corporation-withdraw-credits-btn",
    function(e) {
      var button = $(this);
      var amount = button
        .closest(".modal")
      ...
      $.post("corporation/withdraw_credits", { amount: amount }, function(
        data
      ) {
        button.closest(".modal").modal("hide");
        setTimeout(function() {
  ...
  $("body").on("click", ".corporation-apply-modal-btn", function(e) {
    e.preventDefault();
    if ($(this).data("id")) {
      $.get("/corporation/apply_modal", { id: $(this).data("id") }, function(
        data
      ) {
        $("body").append(data);
        // Enable Popovers
  ...
  $(".corporation-card").on(
    ...
    function() {
      ...
      $.post("corporation/accept_application", { id: id }, function(data) {
        button.closest(".modal").modal("hide");
        $("#corporation-applications-count").html(
          parseInt($("#corporation-applications-count").html()) - 1
        );
        setTimeout(function() {
          load_station_tab("#applications");
        }, 250);
      }).fail(function(data) {
        if (data.responseJSON.error_message) {
          $.notify(data.responseJSON.error_message, { style: "alert" });
...
function load_corporation_tab(href) {
  ...
  element
    .empty()
    .append(
      "<div class='text-center mt-5px'><i class='fa fa-spinner fa-spin fa-2x'></i></div>"
    );
