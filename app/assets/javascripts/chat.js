$(document).on("turbolinks:load", function() {
  ...
  // Join chatroom AJAX
  $("#join-chatroom-modal").on(
    "click",
    "#join-chatroom-modal-join-btn",
    function(e) {
      e.preventDefault();
      ...
      if (id) {
        $.post("/chat/join", { id: id }, function(data) {
          ...
          $("#join-chatroom-modal-join-input")
  ...
  $("#join-chatroom-modal").on(
    "click",
    "#join-chatroom-modal-create-btn",
    function(e) {
      e.preventDefault();
      ...
      if (title) {
        $.post("/chat/create", { title: title }, function(data) {
          ...
          $("#join-chatroom-modal-create-input")
            .removeClass("outline-danger")
            .addClass("outline-danger");
          if (!button.closest(".modal").find(".error").length) {
            button
  ...
  $("#add-to-chat-modal").on("click", "#add-to-chat-modal-search-btn", function(
    e
  ) {
    if ($("#add-to-chat-modal-input").val()) {
      $("#add-to-chat-modal-input").css("border", "1px solid grey");
      $("#add-to-chat-modal-result")
        .empty()
        .append(
      ...
      $.post(
        "/chat/search",
        {
          name: $("#add-to-chat-modal-input").val(),
          identifier: $(this).data("identifier")
        },
        function(data) {
          $("#add-to-chat-modal-body")
            .find(".spinner-modal")
            .remove();
          $("#add-to-chat-modal-result")
            .empty()
...
function removeChatFlash(target) {
  if (target && Cookies.get("chat_flash")) {
    ...
    var flashes = jQuery.grep(JSON.parse(Cookies.get("chat_flash")), function(
      value
    ) {
      return value != target;
    });
  ...
  if ($("#system-player-count").length) {
    ...
    $.each(names, function(key, value) {
      $("#system-players").append(
        "<div><a class='player-modal' href='#' data-id='" +
          value +
          "'>" +
          key +
          "</a></div>"
      ...
      if (fleet == true) {
        $("#" + id + "-players").append(
          "<div><a class='player-modal' href='#' data-id='" +
            ...
            "'>" +
            key +
            "</span></a> <a class='btn btn-outline-primary btn-xs warp-btn float-right text-primary p-0-5' data-uid='" +
            value +
