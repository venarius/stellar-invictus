class DisappearWorker
  ...
  def perform(player_id, remove_logout = false)
    user = User.find(player_id) rescue nil

    if user
      system = user.system

      if (system.low? || system.wormhole?) && (user.online == 1) && !user.logout_timer
        user.update_columns(logout_timer: true)
        DisappearWorker.perform_in(2.minutes, user.id, true)
      else

        # Remove logout timer
        user.update_columns(logout_timer: false) if remove_logout
